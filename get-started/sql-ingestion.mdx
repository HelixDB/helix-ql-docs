---
title: 'Exporting SQL Data and Ingesting into Helix'
...
| `-t`       | `--type`     | Specifies the database type, which is `pg` for PostgreSQL or `sqlite` for SQLite. |
| `-s`       | `--ssl`      | Required for cloud databases like AWS RDS. Not needed for local databases.        |
