...
bundles. Every example shows the **TypeScript**, **Rust**, **Go**, and **JSON** form
side by side, so you can pick a client and follow along — or skim across all four to
see how the surfaces relate.

The four forms are not four different query languages; they are four encodings of
the same query AST. The TypeScript, Rust, and Go DSLs are builders that emit the JSON
shape directly. If you copy the JSON column into a `POST /v1/query` body, it will
execute the same query as the DSL above it.

## Installing the SDKs

To follow along in TypeScript, Rust, or Go, install the SDK for that language. All
three emit the same query AST, so you can switch between them — or use the JSON
...
[Rust Project Setup](/database/rust-project-setup), and
[Go Project Setup](/database/go-project-setup).
...
- The `posts` traversal starts from `NodeRef.var("user")` — that's how one named
  ...
  the JSON.
- The TypeScript, Rust, and Go DSLs surface a friendly `nWithLabel` / `n_with_label` /
...
in four transports — TypeScript, Rust, and Go using their SDK clients, and `curl`
straight from the shell. All four are interchangeable; pick whichever fits your
environment.
...
4. The **Go** tab mirrors the same AST with `helix.ReadQuery` / `helix.WriteQuery`
   dynamic request builders.
5. The **JSON** tab is the serialized AST that every DSL produces. It is the body
