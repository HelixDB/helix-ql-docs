...
(TypeScript), `with_api_key` (Rust), or `WithAPIKey` (Go) and every request the
client sends is authenticated:
...
let client = Client::new(Some("https://helix.example.com"))?.with_api_key(Some(&token));
```

```go Go
import helix "github.com/helixdb/helix-db/sdks/go"

client, err := helix.NewClient("https://helix.example.com", helix.WithAPIKey(token))
if err != nil {
	return err
}
```
...
operations - allowing least-privilege credentials for each service or environment.
