...
## On-Disk with Local MinIO

Use this mode when you want persistent local data without operating your own object store.

### With the CLI

Start an instance with a CLI-managed MinIO sidecar:

```bash
helix start dev --disk
```

To make disk mode the default for an instance, set it during initialization or when adding the
instance:
...
The CLI creates the `helix-db` bucket and preserves the MinIO volume across `helix stop`. Use
`helix prune <instance>` to delete the persisted data.

### With Docker Compose

To manage the local containers yourself, use this Compose file. It starts MinIO, creates the
bucket, and connects `enterprise-dev` to it:
...
Data persists across restarts while the `minio-data` volume exists.

## Existing S3-Compatible Storage

Use this mode when you already operate MinIO, AWS S3, or another S3-compatible object store.
Create the bucket first and give each Helix instance a dedicated prefix. Helix does not create
or delete resources in an externally managed object store.
...
<Warning>Do not commit your `.env` file.</Warning>

Then configure the existing local instance for your provider:
...
<Tabs>
  <Tab title="MinIO">
    ...
    For a plain HTTP endpoint, use an `http://` URL and add `--s3-allow-http`.
  </Tab>
  <Tab title="AWS S3">
    ```bash
    helix start dev \
...
`--persist` writes the resolved storage configuration to `helix.toml`. The same S3 flags are
accepted by `helix init local` and `helix add local`. The CLI loads the project-root `.env` and
forwards AWS credentials on both start and restart.

### With Docker Compose

The equivalent raw container configuration is:
...
For a plain HTTP endpoint, add `AWS_ALLOW_HTTP: "true"`. For AWS S3, remove `AWS_ENDPOINT` and
set `S3_BUCKET`, `S3_REGION`, and `DB_PATH` for your bucket and prefix.

<Note>
The endpoint must be reachable from inside the Helix container. `localhost` refers to the
container itself. `helix stop`, `helix restart`, `helix prune`, and `docker compose down` do not
delete externally managed object-store data.
</Note>
