...
query plus a small envelope that names the request kind and any runtime
parameters. There is no deployment step — the query travels with the request,
so the same DSL that builds it also produces the body you POST.
...
const request = countUsers.toDynamicRequest();
```
