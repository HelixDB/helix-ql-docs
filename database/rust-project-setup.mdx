...
and [Querying](/database/querying) for how the pieces fit together.
...
- Calling the registered function with concrete arguments — e.g. `add_user("u-1".into(),
  "Alice".into())` — returns a `DynamicQueryRequest` you POST to `/v1/query`. See
  [Querying](/database/querying) for the full send path.
- The generated dynamic request sets the top-level `query_name` to the Rust function name, so
  gateway logs and query diagnostics show `add_user` instead of the fallback `__dynamic__`.
...
and aggregations — see the [docs.rs reference](https://docs.rs/helix-db/latest/helix_db/)
and [Querying](/database/querying).
