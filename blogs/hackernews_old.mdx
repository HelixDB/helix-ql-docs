...
5. Deduplicate predicates using an LLM to identify duplicates and hierarchically merge them into semantically unique predicate.
...
However, in practice, we found that RapidFuzz was not always able to identify duplicates accurately,
so we did not include it in our final implementation.
