...
This section is inspired by the [Temporal Agents with Knowledge Graphs](https://github.com/openai/openai-cookbook/blob/a90448358320081935df1a133adb8664bbf75047/examples/partners/temporal_agents_with_knowledge_graphs/temporal_agents_with_knowledge_graphs.ipynb) cookbook from OpenAI. The key difference is in the underlying graph schema and data storage. The original cookbook is built on a Postgres database and a vector database, while we use HelixDB's hybrid graph and vector storage. This allows for more efficient storage without needing to manage multiple databases.
...
Stories and their comment threads are processed through a `Chunker` that uses [Chonkie](https://github.com/chonkie-inc/chonkie)'s `RecursiveChunker` with custom rules to chunk by major sections such as stories, comments, and subcomments, while preserving the hierarchical structure. These chunks flow into the `TemporalAgent`, an asynchronous OpenAI-powered extractor (default model: `gpt-5-nano`) that:
1. Extracts candidate events from each chunk and classifies them as `ATEMPORAL`, `STATIC`, or `DYNAMIC`.
2. Embeds events using OpenAI's `text-embedding-3-small` model for vector search.
3. Assigns temporal validity ranges (`valid_at`/`expired_at`/`invalid_at`) when the text expresses change over time.
4. Extracts entities and triplets (subject‑predicate‑object facts) from the events and uses new predicates or ones from `predicates.csv` where we define existing predicates and their meanings.
...
It groups entities by type, then uses RapidFuzz's `partial_ratio` with an 90% similarity threshold to cluster similar entities. 
Within each cluster, it selects the medoid (entity with highest total similarity to others) as the canonical representative. 
Special acronym detection merges multi-word entities with their acronyms (e.g., "Hacker News" ↔ "HN") using a 98% threshold. 
All duplicate entities are rewired to point at the canonical node via `Resolved` edges, and triplet references are updated accordingly. 
...
The Knowledge Graph Agent is our attempt to combine the power of the retrival agent made by OpenAI's knowledge graph cookbook with HelixDB's graph traversal capabilities. This agent has access to the same tools as the original agent, just using HelixDB's hybrid graph and vector storage instead of a Postgres database and a vector database.
...
Behind the scenes, `KGAgent` coordinates the `Planner` and `Retriever` agent with multi‑step plans, calls the right tools, and returns concise, cited answers. The emphasis is on determinism and auditability: every answer is backed by concrete Events and Triplets.
...
An advantage of storing data in HelixDB is that we can use its MCP tools to directly traverse the graph inside the database.
This allowed us to build a Gemini‑powered retrieval agent that uses HelixDB’s MCP tools for fast, dynamic graph traversals.
...
In a typical flow, Gemini initializes with `mcp/init`, seeds with `mcp/n_from_type` for specific node types, 
chains `mcp/out_step` calls to traverse relationships (e.g., Story → Chunk → Event), 
applies `mcp/filter_items` for temporal or property constraints, and finally `mcp/collect`s the results. 
Because HelixDB optimizes these graph operations are lightweight and the MCP traversal loop remains responsive for agents to iterate on. 
See the [MCP guide](https://docs.helix-db.com/guides/mcp-guide) for more details.
