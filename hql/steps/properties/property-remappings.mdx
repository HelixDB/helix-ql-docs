...
If you want to access a propertis that traverse from the selected element,
...
`nested_field: ID`. This ID would would be the ID of the `other` item, not the `item_name` item due to the tighter scope.
