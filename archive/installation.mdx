...
3. **Define schema and queries**
    Define the [schema and queries](https://helix-llms-txt.mintlify.app/hql-intro.md) in the `schema.hx` and `queries.hx` files in the `db` directory.
...
4. **Build and deploy locally**
    ...
    Check the [CLI commands](https://helix-llms-txt.mintlify.app/cli.md#commands) for more information about the build and deploy process.
    Once Helix CLI is installed and your instance is initialized, continue to [configurations](https://helix-llms-txt.mintlify.app/cli.md#configurations) to customize ports and instances or jump to [HelixDB SDKs](https://helix-llms-txt.mintlify.app/sdks.md) or cURL to test your first query.
