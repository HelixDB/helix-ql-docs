...
For an agent-assisted first app, [`helix go`](/documentation/cli-v2/command-reference/go) can run this setup end-to-end: it installs Helix skills and the docs MCP, initializes `~/my-first-helix-project`, starts `dev`, seeds starter data, and opens the dashboard.
...
<Steps>
  ...
  <Step title="Start the local runtime">
    ...
    For persistent local storage use `helix run dev --disk`, or initialize the project with `helix init local --disk` to make disk mode the default for that instance.
  </Step>
...
[`helix prune`](/documentation/cli-v2/command-reference/prune) only touches Helix-managed containers (`helix-<project>-<instance>` and disk-mode MinIO sidecars), networks, volumes, and the per-instance `.helix/<instance>` directory. It never runs a broad `docker/podman system prune`.
