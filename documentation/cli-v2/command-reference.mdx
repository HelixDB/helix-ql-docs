...
<ResponseField name="helix init [OPTIONS] [SUBCOMMAND]" >
  ...
  <Tip>
  If no subcommand is specified, `helix init` defaults to creating a local instance called `dev`. An interactive prompt will guide you through the setup.
  </Tip>
</ResponseField>
...
<ResponseField name="helix add [SUBCOMMAND]" >
  ...
  <Tip>
  If no subcommand is specified, an interactive prompt will guide you through adding an instance.
  </Tip>
</ResponseField>
...
<Tip>
If no instance is specified, an interactive prompt will let you select one.
</Tip>
...
Downloads `.hx` source files and `helix.toml` configuration from a deployed Helix Cloud instance back to your local project.

<Note>
  Only supported for Helix Cloud instances. You must be authenticated via `helix auth login` before running this command.
</Note>

<ResponseField name="helix sync Optional<INSTANCE>">
  The cloud instance name to sync from.
</ResponseField>

#### Behavior

- Merges `helix.toml` — updates the `[project]` section and the specific `[cloud.<instance>]` block while preserving `[local.*]` and other cloud entries.
- Prompts for confirmation before overwriting any files that differ locally.

#### Examples

```bash
# Sync source files and config from the "production" instance
