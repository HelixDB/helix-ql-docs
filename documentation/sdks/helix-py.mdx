...
When using `local=True`, make sure a local Helix instance is already running (for example via the Helix CLI).
...
Split text into manageable pieces using [Chonkie](https://chonkie.ai/) chunking methods:
