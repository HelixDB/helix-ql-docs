...
<Note>
`FIRST` returns a single element rather than a collection. If the traversal has no results, the binding will be empty.
</Note>
...
### Example 1: Getting the first user
<CodeGroup>
...
### Example 2: Getting the first follower of a user
<CodeGroup>

```rust Query focus={1-4} [expandable]
QUERY GetFirstFollower (user_id: ID) =>
    user <- N<User>(user_id)
