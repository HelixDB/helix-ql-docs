...
description: "Transform and organize traversal results using `COUNT`, `RANGE`, and `ORDER` operations."
icon: "integral"
---
...
```bash Curl [expandable]
# First create many users (repeat for i=0 to 19)
curl -X POST \
  http://localhost:6969/CreateUser \
  -H 'Content-Type: application/json' \
