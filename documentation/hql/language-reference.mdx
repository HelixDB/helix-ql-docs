...
1. A schema with three default values: a number, a string, and an auto-timestamp.
...
1. Loop through a collection and create a node for each element.
...
```helixql lines
#[model("openai:text-embedding-3-small:search_document")]
