...
# First insert some documents to search through
sample_docs = [
...
QUERY SearchRecentKeywords (keywords: String, limit: I64, cutoff_date: Date) =>
    documents <- SearchBM25<Document>(keywords, limit)::WHERE(_::{created_at}::GTE(cutoff_date))
    RETURN documents
...
# Search for documents created in the last 10 days
...
# Insert old documents (15 days ago)
curl -X POST \
  http://localhost:6969/InsertDocument \
  -H 'Content-Type: application/json' \
