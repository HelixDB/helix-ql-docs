...
Select nodes by matching field values instead of ID. This is useful when you need to find nodes with specific property values.
...
<CodeGroup>
```rust Query focus={1-3}
QUERY FindUser (name: String, age: U8) =>
    user <- N<User>({name: name, age: age})
    RETURN user
```
...
<Note>Property-based selection returns all nodes matching the specified field values. For more complex filtering conditions, use [WHERE clauses](../conditionals/conditions).</Note>
