...
QUERY CreateDataCenter(name: String) =>
    datacenter <- CREATE V<DataCenter> { name: name }
    RETURN datacenter

QUERY CreateLink(from_id: ID, to_id: ID, distance: F64, days_old: I64) =>
    link <- CREATE E<Link>(from_id, to_id) {
        distance: distance,
...
QUERY CreateRouter(name: String) =>
    router <- CREATE V<Router> { name: name }
    RETURN router

QUERY CreateCable(from_id: ID, to_id: ID, length: F64, capacity_used: F64) =>
    cable <- CREATE E<Cable>(from_id, to_id) {
        length: length,
