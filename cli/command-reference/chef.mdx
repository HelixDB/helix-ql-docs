...
3. Initializes a local project under `~/my-first-helix-project` with a `dev` instance on port `6969`.
...
Leave this blank to skip. If you provide an app idea, the CLI writes it into `HELIX_CHEF_PROMPT.md` so your coding agent can use it when replacing the generic starter queries.
...
- [`helix run`](/cli/command-reference/run) — start a local instance.
- [`helix query`](/cli/command-reference/query) — send dynamic JSON requests.
- [`helix dashboard`](/cli/command-reference/dashboard) — launch the dashboard.
