...
`helix add cloud` requires V2 Cloud authentication. Run [`helix auth login`](/cli/command-reference/auth) first.
...
# Add a V2 Cloud instance with a known cluster ID
helix add cloud --name production --cluster-id ec_01HX...

# Add a V2 Cloud instance and let helix sync fill in gateway/auth metadata
