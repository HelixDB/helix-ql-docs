...
| `id` | String | No | — | Linked V2 Cloud project ID. Set by [`helix project switch`](/cli/command-reference/project). |
| `workspace_id` | String | No | — | Linked V2 Cloud workspace ID. Set by [`helix project switch`](/cli/command-reference/project). |
...
| `image` | String | `ghcr.io/helixdb/database-dev` | Container image used by [`helix run`](/cli/command-reference/run). |
...
One block per V2 Cloud instance. Most fields here are populated automatically by [`helix sync`](/cli/command-reference/sync).

| Key | Type | Required | Default | Description |
|-----|------|----------|---------|-------------|
| `cluster_id` | String | Yes | — | V2 Cloud cluster ID. List clusters with [`helix cluster list`](/cli/command-reference/cluster). |
...
|----------|---------|-------------|
| `HELIX_API_KEY` (default) | [`helix query`](/cli/command-reference/query) | Per-cluster V2 Cloud API key sent in the configured auth header. The variable name is configurable per-instance via `query_auth_env`. |
